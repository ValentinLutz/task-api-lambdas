...
func NewRestApi(stack awscdk.Stack, config *cdk.StageConfig, database awsrds.DatabaseInstance) awsapigateway.SpecRestApi {
	...
	deleteTaskFunction := cdk.NewJavaFunction(stack, config, "V1DeleteTask", "../lambda-v1-delete-task/target/bootstrap.jar", database)
	getTaskFunction := cdk.NewJavaFunction(stack, config, "V1GetTask", "../lambda-v1-get-task/target/bootstrap.jar", database)
	getTasksFunction := cdk.NewJavaFunction(stack, config, "V1GetTasks", "../lambda-v1-get-tasks/target/bootstrap.jar", database)
	postTasksFunction := cdk.NewJavaFunction(stack, config, "V1PostTasks", "../lambda-v1-post-tasks/target/bootstrap.jar", database)
	putTaskFunction := cdk.NewJavaFunction(stack, config, "V1PutTask", "../lambda-v1-put-task/target/bootstrap.jar", database)
