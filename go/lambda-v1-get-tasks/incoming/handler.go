...
func (handler *Handler) Invoke(ctx context.Context, _ events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	tasks, err := handler.TaskService.GetTasks(ctx)
	if err != nil {
		return events.APIGatewayProxyResponse{}, fmt.Errorf("failed to get tasks: %w", err)
	}

	tasksResponse := NewTasksResponse(tasks)
	tasksResponseBody, err := json.Marshal(tasksResponse)
	if err != nil {
		return events.APIGatewayProxyResponse{}, fmt.Errorf("failed to marshal tasks: %w", err)
	}
